...
if (!url) falha('EXPO_PUBLIC_SUPABASE_URL está vazia', 'Supabase ▸ Settings ▸ Data API ▸ Project URL');
if (!chave) falha('EXPO_PUBLIC_SUPABASE_ANON_KEY está vazia', 'Supabase ▸ Settings ▸ API Keys ▸ anon (ou publishable)');
if (!url || !chave) encerrar();

if (!/^https:\/\/[a-z0-9-]+\.supabase\.co\/?$/.test(url)) {
  alerta(`a URL "${url}" não tem o formato https://xxxx.supabase.co`, 'confira se não colou junto algum caminho a mais');
} else {
...
let resposta;
try {
  resposta = await fetch(`${base}/rest/v1/`, { headers: cabecalhos });
} catch (erro) {
...
if (resposta.status === 401) {
  falha('o projeto respondeu, mas recusou a chave', 'a chave anon/publishable está errada ou é de outro projeto');
  encerrar();
}
if (!resposta.ok) {
...
for (const nome of ['profiles', 'loot_items', 'mural_view']) {
  ...
  if (r.existe) ok(`${nome} existe e está exposta na API`);
  else {
    faltando.push(nome);
    falha(`${nome} não encontrada (${r.status})`, r.mensagem || 'rode o supabase/schema.sql no SQL Editor');
  }
}
...
if (faltando.length === 0) {
  ...
  if (anonimo.linhas === 0) ok('RLS ativo: sem login, o mural não devolve nada');
  else alerta('sem login o mural devolveu linhas', 'as políticas de RLS podem não ter sido criadas — rode a seção 4 do schema.sql');
}
...
try {
  const r = await fetch(`${base}/auth/v1/settings`, { headers: cabecalhos });
  const config = await r.json();

  if (config.disable_signup) {
    falha('o cadastro de novos usuários está desativado', 'Authentication ▸ Sign In / Providers ▸ Allow new users to sign up');
  } else if (config.mailer_autoconfirm === false) {
    alerta(
    ...
    ok('cadastro liberado e "Confirm email" desligado');
  }
} catch {
  alerta('não deu para ler as configurações de autenticação', 'confira manualmente em Authentication ▸ Providers ▸ Email');
}
